...
use cim_domain_person::{
    ...
    cross_domain::{
        CrossDomainEvent, CrossDomainCommand, AddressType,
        IdentityDomainEvent, IdentityEventHandler,
        LocationDomainEvent, LocationEventHandler, AddressUsageType,
        GitDomainEvent, GitEventHandler, LanguageStats,
        AgentDomainEvent, AgentEventHandler, AgentType,
    },
    infrastructure::{
...
async fn test_identity_domain_integration() {
    ...
    let handler = IdentityEventHandler::new(
        person_repo.clone(),
    ...
    let create_events = person.handle_command(PersonCommand::CreatePerson(
...
async fn test_location_domain_integration() {
    ...
    // Create a person
    let person_id = PersonId::new();
    let mut person = Person::new(person_id, PersonName::new("Jane".to_string(), "Smith".to_string()));
    let create_events = person.handle_command(PersonCommand::CreatePerson(
        cim_domain_person::commands::CreatePerson {
            person_id,
...
async fn test_git_domain_integration() {
    ...
    let handler = GitEventHandler::new(
        person_repo.clone(),
        component_store.clone(),
    );
    ...
    // Create a person
    let person_id = PersonId::new();
    let mut person = Person::new(person_id, PersonName::new("Dev".to_string(), "Eloper".to_string()));
    let create_events = person.handle_command(PersonCommand::CreatePerson(
        cim_domain_person::commands::CreatePerson {
            person_id,
...
async fn test_agent_domain_integration() {
    ...
    // Create a person
    let person_id = PersonId::new();
    let mut person = Person::new(person_id, PersonName::new("User".to_string(), "Name".to_string()));
    let create_events = person.handle_command(PersonCommand::CreatePerson(
        cim_domain_person::commands::CreatePerson {
            person_id,
...
async fn test_multi_domain_integration() {
    ...
    // Create handlers
    let identity_handler = IdentityEventHandler::new(
        person_repo.clone(),
        component_store.clone(),
    );
    let location_handler = LocationEventHandler::new(person_repo.clone());
    let git_handler = GitEventHandler::new(
        person_repo.clone(),
        component_store.clone(),
    );
    ...
    // Create a person
    let person_id = PersonId::new();
    let mut person = Person::new(person_id, PersonName::new("Multi".to_string(), "Domain".to_string()));
    let create_events = person.handle_command(PersonCommand::CreatePerson(
        cim_domain_person::commands::CreatePerson {
            person_id,
    ...
    identity_handler.handle_event(IdentityDomainEvent::PersonJoinedOrganization {
        person_id,
    ...
    let address_events = location_handler.handle_event(LocationDomainEvent::AddressAssociatedWithPerson {
    ...
    if !address_events.is_empty() {
        ...
        person_repo.save(&person, address_events, Some(current_version)).await.unwrap();
    }
    
    // 3. Git contributions analyzed
    git_handler.handle_event(GitDomainEvent::ContributionMetricsCalculated {
        person_id,
    ...
    let employment_components = component_store.get_components_by_type(
        person_id,
        ComponentType::Employment,
    ).await.unwrap();
    assert_eq!(employment_components.len(), 1);
    
    let skill_components = component_store.get_components_by_type(
        person_id,
        ComponentType::Skill,
    ).await.unwrap();
