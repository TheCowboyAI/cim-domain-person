...
fn test_component_unregistration() {
    ...
    // Apply event
    person.unregister_component(&ComponentType::PhoneNumber).unwrap();
...
fn test_component_registration_when_inactive() {
    ...
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().to_string(), "Validation error: Cannot add components to inactive person");
...
fn test_component_events() {
    ...
    person.register_component(ComponentType::BehavioralData).unwrap();
